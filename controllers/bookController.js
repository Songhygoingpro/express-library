...
exports.book_detail = asyncHandler(async (req, res, next) => {
  // Get details of the book, its author, genres, and available book instances (in parallel)
...
exports.book_update_get = asyncHandler(async (req, res, next) => {
  ...
  const [book, allAuthors, allGenres] = await Promise.all([Book.findById(req.params.id).populate("author").exec(), Author.find().sort({ family_name: 1 }).exec(), Genre.find().sort({ name: 1 }).exec()]);
